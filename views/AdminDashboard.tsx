...
const AdminDashboard: React.FC<{ user: any }> = ({ user }) => {
  ...
  const handleDeleteProduct = async (e: React.MouseEvent, productId: string) => {
    e.preventDefault();
    e.stopPropagation();
    if (!window.confirm("CRITICAL: Erase this product permanently from the global arsenal?")) return;
  ...
  const handleDeleteOrder = async (e: React.MouseEvent, orderId: string) => {
    e.preventDefault();
    e.stopPropagation();
    if (!window.confirm("WIPE SIGNAL RECORD? This log entry will be deleted forever.")) return;
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 md:py-12 bg-black min-h-screen relative z-[100]">
      <div className="flex flex-col md:flex-row items-center justify-between mb-12 border-b border-white/10 pb-8 gap-6">
        <div className="flex items-center gap-5">
          ...
          <button type="button" onClick={fetchData} className="ml-4 p-2 text-gray-600 hover:text-[#39ff14] transition-colors relative z-[110]">
            <RefreshCw size={20} className={loading ? "animate-spin" : ""} />
          </button>
        </div>

        <div className="flex gap-2 bg-white/5 p-1 border border-white/10 relative z-[110]">
          <button type="button" onClick={() => setView('products')} className={`px-6 py-2 font-orbitron text-[10px] uppercase tracking-widest transition-all ${view === 'products' ? 'bg-[#39ff14] text-black font-black' : 'text-gray-500 hover:text-white'}`}>Manage Arsenal</button>
          <button type="button" onClick={() => setView('orders')} className={`px-6 py-2 font-orbitron text-[10px] uppercase tracking-widest transition-all ${view === 'orders' ? 'bg-[#39ff14] text-black font-black' : 'text-gray-500 hover:text-white'}`}>Signal Logs</button>
        </div>
      </div>
      ...
      <AnimatePresence mode="wait">
        {loading && products.length === 0 ? (
          ...
          <motion.div key="products-view" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            <div className="lg:col-span-1">
              <form onSubmit={handleSubmitProduct} className="bg-white/5 border border-white/10 p-6 space-y-6 sticky top-24 z-20">
                ...
                <div className="space-y-4">
                  ...
                  <input value={tags} onChange={e => setTags(e.target.value)} placeholder="Tags (optional, comma separated)" className="w-full bg-black border border-white/10 p-3 text-xs focus:border-[#39ff14] outline-none text-white uppercase" />
                ...
                <div className="flex gap-2">
                  <button type="submit" disabled={uploading} className="flex-1 bg-[#39ff14] text-black font-orbitron font-black py-4 uppercase tracking-widest text-[10px] hover:brightness-110 transition-all disabled:opacity-50">{uploading ? 'SYNCING...' : 'COMMIT ACTION'}</button>
                  {editingProductId && (
                    <button type="button" onClick={resetForm} className="bg-red-500/10 text-red-500 p-4 border border-red-500/20"><X size={20} /></button>
                  )}
                </div>
              </form>
            </div>
            <div className="lg:col-span-2 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {products.map(product => (
                  <motion.div layout key={product.id} className={`bg-white/5 border p-4 flex gap-4 group transition-all relative z-10 ${editingProductId === product.id ? 'border-[#39ff14]' : 'border-white/10'}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex gap-2 mt-3 relative z-[200]">
                        <button type="button" onClick={(e) => handleEditClick(e, product)} className="flex-1 py-2 bg-white/5 text-[9px] font-orbitron font-black text-gray-500 hover:text-white border border-white/5 uppercase tracking-widest flex items-center justify-center gap-2 pointer-events-auto"><Edit3 size={12} /> Edit</button>
                        <button type="button" onClick={(e) => handleDeleteProduct(e, product.id)} disabled={processingId === product.id} className="flex-1 py-2 bg-red-500/5 text-[9px] font-orbitron font-black text-red-500 hover:bg-red-500 hover:text-white border border-red-500/20 uppercase tracking-widest flex items-center justify-center gap-2 pointer-events-auto transition-colors">
                          {processingId === product.id ? <Loader2 size={12} className="animate-spin" /> : <Trash2 size={12} />} Purge
                        </button>
                      </div>
                    </div>
                  </motion.div>
          ...
          <motion.div key="orders-view" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
            {orders.length === 0 ? (
                ...
                <div key={order.id} className="bg-white/5 border border-white/10 p-6 flex flex-col md:flex-row gap-8 group relative z-10">
                  <div className="flex-1 space-y-4">
                    ...
                    <div className="flex flex-wrap gap-3 pt-4 border-t border-white/5 relative z-[200]">
                      <button type="button" onClick={() => updateOrderStatus(order.id, 'verified')} disabled={processingId === (order.id + 'verified')} className="px-6 py-3 bg-green-500/10 text-green-500 border border-green-500/20 font-orbitron text-[10px] uppercase font-black hover:bg-green-500 hover:text-black transition-all pointer-events-auto">Verify Signal</button>
                      <button type="button" onClick={() => updateOrderStatus(order.id, 'rejected')} disabled={processingId === (order.id + 'rejected')} className="px-6 py-3 bg-red-500/10 text-red-500 border border-red-500/20 font-orbitron text-[10px] uppercase font-black hover:bg-red-500 hover:text-white transition-all pointer-events-auto">Reject Signal</button>
                      <button type="button" onClick={(e) => handleDeleteOrder(e, order.id)} disabled={processingId === order.id} className="px-6 py-3 bg-white/5 text-gray-500 border border-white/10 font-orbitron text-[10px] uppercase font-black hover:bg-red-600 hover:text-white transition-all pointer-events-auto">{processingId === order.id ? <Loader2 size={14} className="animate-spin" /> : <Trash2 size={14} />}</button>
                    </div>
                  </div>
